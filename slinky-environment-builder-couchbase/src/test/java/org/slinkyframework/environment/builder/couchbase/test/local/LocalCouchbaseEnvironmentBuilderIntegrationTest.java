...
public class LocalCouchbaseEnvironmentBuilderIntegrationTest {
    ...
    public static void setUpOnce() throws InterruptedException {
        ...
        // Start a Docker container so that it looks like a local installation
    ...
    public void shouldBeAbleToDoAFreeTextSearch() throws Exception {
        ...
        MatchQuery fts = SearchQuery.match("Zoo");
        SearchQuery query = new SearchQuery(buildDefinition.getFullTextIndexName(), fts)
                .limit(3);

        SearchQueryResult result = bucket.query(query);

        assertThat("Number of results", result.hits().size(), is(equalTo(1)));
